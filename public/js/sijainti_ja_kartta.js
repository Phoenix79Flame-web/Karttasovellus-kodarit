...
if (paikka_merkitty == 0) {
  ...
  olet_tassa.bindPopup("<b> Olet tässä </b><br>" + "halutako merkitä paikan?").openPopup();
}
}
...
function laheta_arvostelu() {
  ...
  console.log("Lähetä arvostelu options; " , options);

  fetch('api/arvostelu', options).then(function(response) {
    console.log("Lähetä arvostelu response: ", response)
